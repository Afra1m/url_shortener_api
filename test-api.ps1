...
if ($shortCodes.Count -gt 0) {
    # Test only first one
    $testCode = $shortCodes[0]
    try {
        $response = Invoke-RestMethod -Uri "http://localhost/stats/$testCode" -Method GET -UseBasicParsing
        Write-Host "   ✓ Stats for $testCode: Available" -ForegroundColor Green
        Write-Host "      Visits: $($response.visits)" -ForegroundColor Gray
    }
    catch {
        Write-Host "   ! Stats for $testCode: Error" -ForegroundColor Yellow
    }
}
